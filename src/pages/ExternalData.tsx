...
export default function ExternalData() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "hide_archived" | "available" | "active" | "archived"
  >("hide_archived");
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (statusFilter !== "all") {
        const [activeRes, logsRes] = await Promise.all([
          supabase
            ...
            .not("linked_external_id", "is", null),
          supabase
            .from("candidate_logs")
            .select("linked_external_id")
            .not("linked_external_id", "is", null),
      ...
      toast({
        title: "Gagal",
        description:
          "Gagal menarik data dari Supabase. Pastikan tabel external_data sudah dibuat.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleDelete = async (row: any) => {
    if (!webhookDeleteUrl) {
      toast({
        title: "Peringatan",
        description: "URL Webhook DELETE belum diatur di Pengaturan.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (!rowIdentifier) {
      toast({
        title: "Error",
        description:
          "Tidak dapat menemukan identitas baris (uid_sheet) pada data ini.",
        variant: "destructive",
      });
    ...
    try {
      ...
      const response = await fetchWithRetry(
        ...
        1,
      ); // Set maxRetries to 1 to avoid long retries on timeout
      ...
      if (response.ok) {
        ...
        toast({
          title: "Berhasil",
          description:
            "Permintaan hapus data telah dikirim dan file lampiran akan dihapus. Periksa notifikasi untuk status selanjutnya.",
        });
      ...
      if (err.name === "AbortError") {
        toast({
          title: "Waktu Habis",
          description:
            "Server n8n terlalu lama merespons (lebih dari 30 detik).",
          variant: "destructive",
        });
      } else {
        toast({
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      await printElement(
        ...
        `Data_Pelamar_${previewData?.full_name || "Kandidat"}`,
      );
      toast({
        title: "Berhasil",
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 px-4 text-center bg-white rounded-2xl border border-slate-200 shadow-sm border-dashed">
          ...
          <p className="text-slate-500 max-w-md mx-auto">
            Data dari Supabase masih kosong. Pastikan Workflow n8n Anda sudah
            berjalan dan menyinkronkan data dari Google Sheet ke Supabase.
          </p>
        </div>
      ) : (
        <>
          <div className="relative">
                      ...
                      <button
                        ...
                        title={
                          row._link_status === "active"
                            ...
                            : row._link_status === "archived"
                              ? "Tidak dapat menghapus data yang sudah diarsipkan"
                              : "Hapus Data"
                        }
                      >
                        <Trash2 size={14} />
            ...
            {totalItems > 0 && (
              <div className="flex flex-col sm:flex-row items-center justify-between bg-white/60 backdrop-blur-md px-4 py-3 border border-white/60 rounded-2xl shadow-xl gap-4 mt-8 relative z-10">
                ...
                <div>
                  <nav
                    className="relative z-0 inline-flex rounded-xl shadow-sm -space-x-px overflow-hidden"
                    ...
                    {totalPages <= 7 ? (
                      ...
                      <>
                        ...
                        {currentPage !== 1 && currentPage !== totalPages && (
                          <button className="relative inline-flex items-center px-4 py-2 border text-sm font-medium z-10 bg-indigo-50/80 backdrop-blur-md border-indigo-200 text-indigo-600">
                            {currentPage}
                          </button>
                        )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-slate-100 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[95vh] flex flex-col overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-8 bg-slate-100/80 flex justify-center items-start custom-scrollbar">
              <div
                ref={componentRef}
                className="w-full max-w-4xl bg-white shadow-2xl shrink-0 rounded-2xl overflow-hidden print:shadow-none print:rounded-none"
              >
                <ApplicationForm
                  readOnly
