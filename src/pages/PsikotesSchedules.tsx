...
export default function PsikotesSchedules() {
  ...
  useEffect(() => {
    if (highlightId) {
      ...
      // Try to scroll to it after a short delay to ensure rendering
      setTimeout(() => {
  ...
  return (
    <div className="space-y-8">
      ...
      {activeTab === 'pending' && (
        <div className="space-y-4">
          <div className="flex items-center gap-3 px-2">
            <h2 className="text-xl font-bold text-slate-800">Agenda Psikotes</h2>
            ...
            {schedules.map((schedule) => (
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className={cn(
                    ...
                    schedule.location_type === 'online' ? "bg-indigo-100 text-indigo-700" : "bg-amber-100 text-amber-700"
                  )}>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex gap-1">
                      ...
                      <button 
                        ...
                        title={schedule.is_confirmed ? "Batalkan Konfirmasi" : "Konfirmasi Sudah Psikotes"}
                      >
      ...
      {activeTab === 'confirmed' && (
        <div className="space-y-4">
          <div className="flex items-center gap-3 px-2">
            <h2 className="text-xl font-bold text-slate-800">Daftar Psikotes Selesai</h2>
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <Link to={`/candidates/${schedule.candidate_id}`} className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold text-xs hover:bg-indigo-200 transition-colors">
                            {schedule.candidate?.full_name[0]}
                          </Link>
                          <div>
                            <p className="font-bold text-slate-900 text-sm">
                              <Link to={`/candidates/${schedule.candidate_id}`} className="hover:text-indigo-600 transition-colors">
                                {schedule.candidate?.full_name}
                              </Link>
                            </p>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-100 shrink-0">
              <h2 className="text-lg sm:text-xl font-bold text-slate-900">Detail Jadwal Psikotes</h2>
                ...
                <div className="flex-1 space-y-6 bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-100">
                  <div className="space-y-4">
                    <div className="flex items-start gap-3">
                      <CalendarIcon className="text-indigo-500 mt-0.5 shrink-0" size={18} />
                      <div>
                    ...
                    <div className="flex items-start gap-3">
                      {previewSchedule.location_type === 'online' ? <Globe className="text-indigo-500 mt-0.5 shrink-0" size={18} /> : <MapPin className="text-indigo-500 mt-0.5 shrink-0" size={18} />}
                      <div className="min-w-0">
            ...
            <div className="p-4 sm:p-6 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row gap-3 shrink-0">
              ...
              >
                ...
                <span className="truncate">{previewSchedule.is_confirmed ? 'Batalkan Konfirmasi' : 'Konfirmasi Sudah Psikotes'}</span>
              </button>
              <button 
                onClick={() => setPreviewSchedule(null)}
      ...
      <ConfirmModal
        ...
        message={
          confirmModal.type === 'delete'
            ? 'Apakah Anda yakin ingin menghapus jadwal psikotes ini? Tindakan ini tidak dapat dibatalkan.'
            : `Apakah Anda yakin ingin ${!confirmModal.currentStatus ? 'mengonfirmasi bahwa kandidat ini sudah melakukan psikotes' : 'membatalkan konfirmasi psikotes'}?`
        }
