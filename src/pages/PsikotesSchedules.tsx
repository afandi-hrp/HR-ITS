...
export default function PsikotesSchedules() {
  ...
  useEffect(() => {
    if (highlightId) {
      ...
      setTimeout(() => {
        document
          .getElementById(`schedule-${highlightId}`)
  ...
  const handleConfirm = async (schedule: any) => {
    if (!schedule.is_confirmed) {
      ...
      if (currentTime < scheduleTime) {
        toast({
          ...
          description:
            "Waktu psikotes belum berlalu. Anda belum bisa mengkonfirmasi kehadiran.",
          variant: "destructive",
        });
        return;
      }
    ...
    setConfirmModal({
      isOpen: true,
      id: schedule.id,
      currentStatus: schedule.is_confirmed,
      type: "confirm",
      loading: false,
    });
  };
      ...
      if (error) {
        ...
        toast({
          ...
          description: !currentStatus
            ? "Kandidat dikonfirmasi sudah psikotes."
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex gap-1">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg transition-colors text-white bg-rose-600 hover:bg-rose-700"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
                ...
                {schedules.length === 0 && !loading && (
                  <div className="col-span-full text-center py-20 bg-white rounded-2xl border border-dashed border-slate-300">
                    ...
                    <p className="text-slate-500">
                      Semua jadwal telah dikonfirmasi atau belum dibuat.
                    </p>
                  </div>
                )}
              </div>
                      ...
                      {schedules.map((schedule) => (
                        <tr
                          ...
                          className={cn(
                            ...
                            blinkingId === schedule.id
                              ? "animate-pulse bg-indigo-50/50 ring-2 ring-indigo-500 ring-inset"
                              : "",
                          )}
                        >
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              <Link
                                to={`/candidates/${schedule.candidate_id}`}
                                className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold text-xs hover:bg-indigo-200 transition-colors"
                              >
              ...
              <div className="flex flex-col md:flex-row gap-6 sm:gap-8">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-sm text-slate-500 truncate">
                          {previewSchedule.candidate?.email}
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-[#5A305A]">
                      ...
                      <span className="font-medium truncate">
                        {previewSchedule.candidate?.position}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="flex-1 space-y-6 bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-100">
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start gap-3">
                      {previewSchedule.location_type === "online" ? (
                        <Globe
                          className="text-indigo-500 mt-0.5 shrink-0"
                          size={18}
                        />
              ...
              <button
                onClick={() => {
                  if (previewSchedule.candidate) {
                    setEmailModalData({
                      candidate: previewSchedule.candidate,
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          if (confirmModal.type === "delete") {
            ...
            executeConfirm(
              confirmModal.id,
              confirmModal.currentStatus || false,
            );
          }
        }}
        title={
        ...
        confirmText={
          confirmModal.type === "delete" ? "Hapus" : "Ya, Konfirmasi"
        }
