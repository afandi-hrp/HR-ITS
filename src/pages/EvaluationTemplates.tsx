...
export default function EvaluationTemplates() {
  ...
  const fetchTemplates = async () => {
    setLoading(true);
    let query = supabase.from("evaluation_templates").select("*");
  ...
  const handleSave = async () => {
    ...
    if (scales.length === 0) {
      toast({
        title: "Peringatan",
        description: "Minimal harus ada 1 skala penilaian.",
        variant: "destructive",
      });
      return;
    }
    if (
      categories.length === 0 ||
      categories.some((c) => c.criteria.length === 0)
    ) {
      toast({
        title: "Peringatan",
        description: "Kategori dan kriteria tidak boleh kosong.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (error) {
      ...
      toast({
        title: "Berhasil",
        description: `Template evaluasi berhasil ${editingTemplate ? "diperbarui" : "disimpan"}.`,
      });
  ...
  const executeDelete = async (id: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast({
          title: "Berhasil",
          description: "Template evaluasi berhasil dihapus.",
        });
        fetchTemplates();
      }
    } finally {
  ...
  // Scale
  const addScale = () =>
    setScales([...scales, { score: scales.length + 1, label: "" }]);
  ...
  const addCategory = () =>
    setCategories([
      ...categories,
      { name: "", criteria: [{ name: "", description: "" }] },
    ]);
  ...
  const addSummaryField = () =>
    setSummaryFields([
      ...summaryFields,
      { name: "", type: "textarea", placeholder: "" },
    ]);
                    ...
                    {scales.map((scale, idx) => (
                      ...
                      >
                        <input
                          ...
                          value={scale.score}
                          onChange={(e) =>
                            updateScale(idx, "score", e.target.value)
                          }
                        ...
                        <input
                          ...
                          value={scale.label}
                          onChange={(e) =>
                            updateScale(idx, "label", e.target.value)
                          }
                      ...
                      >
                        ...
                        <div className="p-4 space-y-3">
                          {cat.criteria.map((crit, critIdx) => (
                            <div
                              key={critIdx}
                              className="flex gap-3 bg-white p-3 rounded-xl border border-slate-200 shadow-sm"
                            >
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                  ...
                  <div className="space-y-4">
                    {summaryFields.map((field, idx) => (
                      <div
                        key={idx}
                        className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-col md:flex-row gap-4"
                      >
                        <div className="flex-1 space-y-3">
                          <div className="flex gap-3">
                            <input
                              ...
                              value={field.name}
                              onChange={(e) =>
                                updateSummaryField(idx, "name", e.target.value)
                              }
                            ...
                            <select
                              value={field.type}
                              onChange={(e) =>
                                updateSummaryField(idx, "type", e.target.value)
                              }
                          ...
                          {field.type === "textarea" ? (
                            ...
                            <div className="space-y-1">
                              <input
                                ...
                                placeholder={
                                  field.type === "scoring"
                                    ? "Masukkan item scoring, pisahkan dengan koma (Cth: Lokasi, Jenjang Karir)"
                                    : "Masukkan opsi, pisahkan dengan koma (Cth: Ya, Tidak, Mungkin)"
                                }
                                className="w-full px-3 py-2 text-sm bg-white border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                              />
